...
public class AnalyticsActivity extends Activity {
	...
	private HashMap<String, Integer> states = null, county = null,
			cities = null;
	ArrayAdapter<String> statesadapter = null, countyadapter = null,
			citiesadapter = null;
	LinearLayout ll;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		statesadapter = new ArrayAdapter<String>(getApplicationContext(),
				R.layout.dropdown, stateList);
		countyadapter = new ArrayAdapter<String>(getApplicationContext(),
				R.layout.dropdown, countyList);
		citiesadapter = new ArrayAdapter<String>(getApplicationContext(),
				R.layout.dropdown, cityList);
		...
		state.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
				...
				countycode = -1;
				new MyTask((AppConstants.URL1 + "/data/?p1=" + statecode),
						county, countyList).execute(((Void) null));
		...
		new MyTask((AppConstants.URL1 + "/data"), states, stateList)
				.execute(((Void) null));
		...
		apply.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent intent = new Intent(AnalyticsActivity.this,
						TabsActivity.class);
				...
				if (year.getSelectedItem().toString()
						.equalsIgnoreCase("-Year-"))
					intent.putExtra("year", -1);
				else
					intent.putExtra("year",
							Integer.parseInt(year.getSelectedItem().toString()));
				if (month.getSelectedItem().toString()
					...
					ArrayList<String> arrayList = new ArrayList<String>(Arrays
							.asList(ar));
					intent.putExtra("month", arrayList.indexOf(month
	...
	private class MyTask extends AsyncTask<Void, Void, Boolean> {
		...
		public MyTask(String url, HashMap<String, Integer> map,
				ArrayList<String> list) {
			super();
			this.url = url;
			this.map = map;
				...
				try {
					...
					if (ar.length() > 1) {
						...
						for (int i = 0; i < items.length(); i++) {
							...
							if (map == states) {
								// Log.d("Map", "states");
								map.put(obj.getString("sname"), Integer
										.parseInt(obj.getString("scode")));
								list.add(obj.getString("sname"));
							} else if (map == county) {
								map.put(obj.getString("name"), Integer
										.parseInt(obj.getString("county")));
								list.add(obj.getString("name"));
								// Log.d("Map", "County");
							} else {
								map.put(obj.getString("name"),
										Integer.parseInt(obj.getString("city")));
								list.add(obj.getString("name"));
								// Log.d("Map", "cities");
							}
						}
